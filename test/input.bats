...
@test "does not modify pipe chars inside strings" {
  ...
  pipespected_output=$(../pipespect.sh -q "echo '[ { \"values\": [0] }, { \"values\": [ 1, 2 ] } ]' | jq '[.[].values[]] | length'")
...
@test "handles pipes without surrounding whitespace" {
  ...
  pipespected_with_spaces_output=$(../pipespect.sh -q "echo 'foo' | sed 's/foo/bar/g'" 2>&1 >/dev/null)
  pipespected_without_spaces_output=$(../pipespect.sh -q "echo 'foo'|sed 's/foo/bar/g'" 2>&1 >/dev/null)
